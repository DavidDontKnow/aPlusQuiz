...
function displayQuestion() {
    let currentQuestionInfo = questions[index];
    quizArea.innerHTML = `<h1>Question ${index + 1}</h1>`
    quizArea.appendChild(question);
    question.innerHTML = `<p>${currentQuestionInfo.question}</p>`;
    for (let choice in currentQuestionInfo.choices) {
        question.innerHTML += `<button class="choice" value="${choice}">${currentQuestionInfo.choices[choice]}</button>`;
    };
}
...
function checkAnswer() {
    ...
    if (choice === currentQuestionInfo.answer) {
        score++;
    } else {
        score--;
    }
...
function endQuiz() {
    quizArea.innerHTML = `<h2>Quiz Complete!</h2>
    <h3>Your score is ${score} out of ${totalQuestions}</h3>`;
    quizArea.innerHTML += `<button class="startButton">Start Quiz Again</button>`;
    return
}